...
export const ROOT=resolve(import.meta.dirname,'..'),DATA=resolve(process.env.PIPELINE_DATA_DIR??join(ROOT,'data')),RUNS=join(DATA,'runs'),UPLOADS=join(DATA,'uploads');
...
export function save(p:string,v:any){const tmp=p+'.'+process.pid+'.'+crypto.randomUUID()+'.tmp';writeFileSync(tmp,JSON.stringify(v,null,2)+'\n');renameSync(tmp,p)}
