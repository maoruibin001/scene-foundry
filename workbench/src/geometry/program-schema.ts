...
所有坐标以米为单位，Z 向上。position 为局部原点偏移，rotation 为绕 X、Y、Z 依次旋转的弧度，scale 为正数缩放。shape 坐标先经过部件的旋转、缩放，再加上部件 position，最后应用实例变换；不要把 shape 自身已有的高度再次作为 position。尤其 lathe.profile 的 [半径, 高度] 是局部 Z 值：若轮廓从 0 到 4.5 且模板底部为 Z=0，part.position.z 应为 0 而不是 2.25。每个模板的最终全部部件须落在冻结的 bounds 内。名称、标签使用中文；ID 用字母开头的字母数字下划线。requirementIds 仅引用给出的原始需求。
